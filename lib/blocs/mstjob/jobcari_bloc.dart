...
class JobCariBloc extends Bloc<JobCariEvents, JobCariState> {
	...
	Future<void> onFetchJobCari(
		...
		if (state.status == ListStatus.initial) {
			List<JobCariModel> items = await repo.getJobCari(event.searchText, 0);
		...
		List<JobCariModel> items = await repo.getJobCari(event.searchText, state.hal);
		if (items.isEmpty) {
			...
			final result = jobCari
				.whereWithIndex((e, index) =>
					jobCari.indexWhere((e2) => e2.mjobId == e.mjobId) ==
					index)
				.toList();
