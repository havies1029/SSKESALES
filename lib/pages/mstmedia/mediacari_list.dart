...
class MediaCariPageState extends State<MediaCariPage> {
	...
	void refreshData() {
		mediaCariBloc.add(
			RefreshMediaCariEvent(searchText: _searchController.text, hal: 0));
	}
	...
	IconButton buildSearchButton() {
		return IconButton(
			...
			onPressed: () {
			mediaCariBloc.add(RefreshMediaCariEvent(
	...
	Widget buildList() {
		return Expanded(
			child: Column(
				mainAxisAlignment: MainAxisAlignment.start,
				children: <Widget>[MediaCariListWidget(searchText: _searchController.text)],
		));
	}
