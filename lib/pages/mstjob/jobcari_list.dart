...
class JobCariPage extends StatefulWidget {
  ...
  const JobCariPage({super.key, required this.personId});
...
class JobCariPageState extends State<JobCariPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
        listeners: [
          BlocListener<JobCariBloc, JobCariState>(listener: (context, state) {
          ...
          BlocListener<JobCrudBloc, JobCrudState>(listener: (context, state) {
            if (state.isSaved) {
              refreshData();
            }
          }, listenWhen: (previous, current) {
        ...
        child: Scaffold(
          floatingActionButton: readOnly?null:
  ...
  void refreshData() {
    jobCariBloc.add(RefreshJobCariEvent(
        searchText: _searchController.text, hal: 0, personId: widget.personId));
  }
